...
export default function SignUp() {
  ...
  const validateForm = () => {
    ...
    if (formData.password.length < 6) {
      toast.error("Password must be at least 6 characters", { toastId: 'password-length-error' });
      return false;
    }
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
          ...
          <div>
            <button
              type="submit"
              disabled={loading}
              className="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-[#FF7D29] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-70 disabled:cursor-not-allowed hover:opacity-70"
            >
              {loading ? (
                <>
